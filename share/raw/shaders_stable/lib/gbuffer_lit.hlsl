...
// The output of the pixel shader will ne used in a compute shader that handles lighting, reflections etc.
GBufferPixelOutput ps_main(const GBufferPixelInput pixel, const uint isFrontFace : SV_IsFrontFace)
{
	GBufferPixelOutput output;
	output.albedo = gbuffer_calculate_albedo(pixel, isFrontFace);
	output.normalGloss = gbuffer_calculate_normal_gloss(pixel, isFrontFace);
	output.reflectanceOcclusion = gbuffer_calculate_reflectance_occlusion(pixel, isFrontFace, output.albedo);
	return output;
}